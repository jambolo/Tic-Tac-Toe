...
TEST(TicTacToeState, Constuctor_default)
{
    ...
    ValidateState(state, Board(), TicTacToeState::PlayerId::FIRST, false, false, Board::Cell::NEITHER);
...
{
    ASSERT_NO_THROW(TicTacToeState(boardDraw, TicTacToeState::PlayerId::SECOND));

    {
        ASSERT_NO_THROW(TicTacToeState(board0, TicTacToeState::PlayerId::FIRST));
        TicTacToeState state(board0, TicTacToeState::PlayerId::FIRST);
        ValidateState(state, board0, TicTacToeState::PlayerId::FIRST, false, false, Board::Cell::NEITHER);
    }
    {
        ASSERT_NO_THROW(TicTacToeState(boardNotDone, TicTacToeState::PlayerId::FIRST));
        TicTacToeState state(boardNotDone, TicTacToeState::PlayerId::FIRST);
        ValidateState(state, boardNotDone, TicTacToeState::PlayerId::FIRST, false, false, Board::Cell::NEITHER);
    }
    {
        ASSERT_NO_THROW(TicTacToeState(boardXWins, TicTacToeState::PlayerId::SECOND));
        TicTacToeState state(boardXWins, TicTacToeState::PlayerId::SECOND);
        ValidateState(state, boardXWins, TicTacToeState::PlayerId::SECOND, true, false, Board::Cell::X);
    }
    {
        ASSERT_NO_THROW(TicTacToeState(boardOWins, TicTacToeState::PlayerId::FIRST));
        TicTacToeState state(boardOWins, TicTacToeState::PlayerId::FIRST);
        ValidateState(state, boardOWins, TicTacToeState::PlayerId::FIRST, true, false, Board::Cell::O);
    }
    {
        ASSERT_NO_THROW(TicTacToeState(boardDraw, TicTacToeState::PlayerId::SECOND));
        TicTacToeState state(boardDraw, TicTacToeState::PlayerId::SECOND);
        ValidateState(state, boardDraw, TicTacToeState::PlayerId::SECOND, true, true, Board::Cell::NEITHER);
    }
}
...
TEST(TicTacToeState, WhoseTurn)
{
    {
        TicTacToeState state;
        EXPECT_EQ(state.whoseTurn(), TicTacToeState::PlayerId::FIRST);

        // X makes a move marking (1, 1)
        state.move(1, 1);
        EXPECT_EQ(state.whoseTurn(), TicTacToeState::PlayerId::SECOND);
    }
    {
        TicTacToeState state(Board(), TicTacToeState::PlayerId::SECOND);
        EXPECT_EQ(state.whoseTurn(), TicTacToeState::PlayerId::SECOND);
...
{
    {
        ...
        ValidateState(state, board, TicTacToeState::PlayerId::SECOND, false, false, Board::Cell::NEITHER);
    ...
    {
        TicTacToeState state(boardDrawNext, TicTacToeState::PlayerId::FIRST);
        ValidateState(state, boardDrawNext, TicTacToeState::PlayerId::FIRST, false, false, Board::Cell::NEITHER);
...
{
    EXPECT_EQ(TicTacToeState::toCell(TicTacToeState::PlayerId::FIRST), Board::Cell::X);
    EXPECT_EQ(TicTacToeState::toCell(TicTacToeState::PlayerId::SECOND), Board::Cell::O);
}
...
TEST(TicTacToeState, ToPlayerId)
{
    ...
    EXPECT_EQ(TicTacToeState::toPlayerId(Board::Cell::X).value(), TicTacToeState::PlayerId::FIRST);
    EXPECT_EQ(TicTacToeState::toPlayerId(Board::Cell::O).value(), TicTacToeState::PlayerId::SECOND);
}
} // namespace TicTacToe
